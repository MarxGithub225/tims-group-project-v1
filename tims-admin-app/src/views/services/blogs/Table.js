    ...
    <div className='invoice-list-table-header w-100 mr-1 ml-50 mt-2 mb-75'>
      <Row>
        ...
        >
          ...
          <Button.Ripple color='primary' onClick = {() => history.push(`/service/blogs/new`)}>
...
const DataTableServerSide = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cell: row => (
          <span className='font-weight-bold'>{row?.viewsCount}</span>
      )
    },
    {
      name: 'Durée',
      ...
      sortable: true,
      cell: row => (
          <span className='font-weight-bold'>{formatDuration(Number(row?.videoDuration) + Number(row?.readDuration)*60)}</span>
      )
    },
    {
      name: 'Etat',
    ...
    ,
    {
      ...
      sortable: true,
      cell: row => (
        <span className='font-weight-bold'>{row?.likes?.length} / {row?.comments?.length}</span>
      )
    },
    {
      name: 'Actions',
      ...
      cell: row => (
        ...
        <Button
          color="flat-primary"
          onClick = {() => history.push(`/service/blogs/edit/${row?._id}`)}
        >
          <ArrowRight size={15} />
        </Button>
    ...
    <Fragment>
      ...
      <Card>
        <CardBody>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DES BLOGS ({(store.total ?? 0) }) </h1>
        </CardBody>
      </Card>
      <Card>
