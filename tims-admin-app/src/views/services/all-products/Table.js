...
const DataTableServerSide = () => {
  ...
  const store = useSelector(state => state.allproducts)
  ...
  // ** Get data on mount
  useEffect(() => {
    dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm, allProduct: true}))
  ...
  const handleFilter = val => {
    ...
    timer = setTimeout(() => {
      dispatch(getAllData({limit: rowsPerPage, page: 1, search: val, allProduct: true}))
    }, 1000)
    ...
    <Fragment>
      ...
      <Card>
        <CardBody>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DE TOUS NOS ARTICLES ({(store.total ?? 0) }) </h1>
        </CardBody>
      </Card>
      <Card>
