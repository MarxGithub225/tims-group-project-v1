...
// ** Store & Actions
import { getAllData, searchBrand } from './store/action'
...
const columns = [
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex justify-content-left align-items-center'>
        <span className='font-weight-bold'>{row.isPartner ?'Oui': 'Non'}</span>
      </div>
    )
  },
  {
...
const handleDeleteConfirmation = async ({_id}) => {
  return MySwal.fire({
    ...
    if (result.value) {
     ...
     axios.delete(`${base_url}brand/delete/${_id} `, config).then(() => {
      dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm}))
      toast.success(
        <Fragment>
  ...
  return (
    <Fragment>
        <Card>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DES MARQUES ({(store.total ?? 0) }) </h1>
        </CardBody>
        </Card>
      <Card>
        <DataTable
