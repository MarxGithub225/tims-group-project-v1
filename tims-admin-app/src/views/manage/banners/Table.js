...
const columns = [
  ...
  },
  {
    ...
    >
        ...
        {row.type === "main-banner" ? 'Large bannière' : 'Bannière de produit'}
    </div>
  ...
  },
  {
    ...
    sortable: true,
    cell: row => (
      <Badge className='text-capitalize' color={ new Date(row.startAt).getTime() <= new Date().getTime() && new Date().getTime() <= new Date(row.endAt).getTime() ?
        (!row.isBlocked ? 'light-success' : 'light-secondary'): 'light-warning'} pill>
        
        {new Date(row.startAt).getTime() <= new Date().getTime() && new Date().getTime() <= new Date(row.endAt).getTime() ?
        (!row.isBlocked ? 'actif' : 'inactif') : 'Expirée'}
      </Badge>
    )
  },
  {
