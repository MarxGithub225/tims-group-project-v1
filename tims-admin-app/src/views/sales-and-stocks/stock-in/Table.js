...
const DataTableServerSide = () => {
  ...
  // ** Get data on mount
  useEffect(() => {
    dispatch(getAllData({limit: rowsPerPage, page: currentPage, agenceToId: userData?.currentAgency, transferType: 'in' }))
  ...
  const columns = [
    ...
    {
      name: 'De',
      selector: "agenceFromId",
      minWidth: '20%',
      sortable: true,
      cell: row => (
        <span className='font-weight-bold'>{row.agenceFromId?.name}</span>
      )
    },
    {
      name: 'Actions',
