...
// ** Store & Actions
import { getAllData, searchAgency } from './store/action'
import { useDispatch, useSelector } from 'react-redux'

// ** Third Party Components
import ReactPaginate from 'react-paginate'
import { ChevronDown, Check, Edit, Trash, AlertOctagon } from 'react-feather'
...
const AgencyList = () => {
  ...
  const [sidebarComercialOpen, setSidebarCommercialOpen] = useState(false)
  ...
  const toggleSidebarCommercial = () => setSidebarCommercialOpen(!sidebarComercialOpen)
  const closeSidebar = () => {
...
const handleDeleteConfirmation = async ({_id}) => {
  return MySwal.fire({
    ...
    if (result.value) {
     ...
     axios.delete(`${base_url}agency/delete/${_id} `, config).then(() => {
      dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm}))
      toast.success(
        <Fragment>
...
const columns = [
  ...
  },
  {
    ...
    cell: row => (
      ...
      <div className='d-flex flex-column'>
      <Link
        to={`/apps/user/view/${row?.managerId && row?.managerId?._id}`}
        className='user-name text-truncate mb-0'
      >
        <span className='font-weight-bold'>{row?.managerId && row?.managerId?.fullName}</span>
      </Link>
      <small className='text-truncate text-muted mb-0'>@{row?.managerId && row?.managerId?.email}</small>
      </div>
    ...
    cell: row => (
      <div className="d-flex">
        ...
        <Button
          ...
          onClick={() => {
            history.push(`/manage/show-rooms/stock-verification/${row._id}`)
          }}
        >
  ...
  return (
    <Fragment>
      <Card>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DES SHOW ROOMS ({( store.total ?? 0) }) </h1>
          </CardBody>
        </Card>
      <Card>
      <DataTable
