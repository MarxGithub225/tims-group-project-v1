...
// ** Store & Actions
import { getAllData, searchSubcategory } from './store/action'
...
const SubCategoryList = () => {
  ...
  const store = useSelector(state => state.subcategories)
  ...
  // ** Get data on mount
  useEffect(() => {
    dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm, productCategoryId: id}))
  ...
  const handleFilter = val => {
    setSearchTerm(val)
    dispatch(searchSubcategory(val))
  ...
  const handleDeleteConfirmation = async ({_id}) => {
    ...
    return MySwal.fire({
      ...
      if (result.value) {
       ...
       axios.delete(`${base_url}subcategory/delete/${_id} `, config).then(() => {
  ...
  return (
    <Fragment>
        ...
        <Card>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DES SOUS CATEGORIES ({(store.total ??  0) }) </h1>
          </CardBody>
        </Card>
      <Card>
      <DataTable
