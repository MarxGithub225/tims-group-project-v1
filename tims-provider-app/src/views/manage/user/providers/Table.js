...
import { ChevronDown, MoreVertical, FileText, Trash2, Archive, Check, Edit, Trash } from 'react-feather'
...
const renderClient = row => {
  const stateNum = Math.floor(Math.random() * 6),
    ...
    return <Avatar color={color || 'primary'} className='mr-1' content={row.companyName ?? ''} initials />
}
...
const columns = [
  {
    ...
    cell: row => (
      <div className='d-flex justify-content-left align-items-center'>
        ...
        <span className='font-weight-bold'>{row.companyName}</span>
        </div>
      </div>
    )
  },
