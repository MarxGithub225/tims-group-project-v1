...
const DataTableServerSide = () => {
  ...
  const store = useSelector(state => state.archivedproducts)
  ...
  // ** Get data on mount
  useEffect(() => {
    dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm, archived: 'archived'}))
  ...
  const handleFilter = val => {
    ...
    timer = setTimeout(() => {
      dispatch(getAllData({limit: rowsPerPage, page: 1, search: val, archived: 'archived'}))
    }, 1000)
  }
  let refuseTranslate = {
  ...
  const handleConfirmation = async (row) => {
    ...
    return MySwal.fire({
      ...
      confirmButtonText: 'Oui, enlever des archives !',
      ...
      if (result.value) {
        setLoading(true)
        await axios.put(`${base_url}product/update/${row?._id}`, {...row, isArchived: false, isUpdated: true, approved: false}, config).then(() => {
        dispatch(getAllData({limit: rowsPerPage, page: currentPage, search: searchTerm, archived: 'archived'}))
        toast.success(
          <Fragment>
    ...
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center' >
         <Badge className='text-capitalize' color={`${(row.isNew || row.isUpdated || row?.isArchived) ? 'light-secondary': `${row.cancelled ? 'light-danger' :row.isBlocked ? 'light-warning' :'light-success'}`}`} pill>
          {
            (row.isNew || row.isUpdated) ? 'En attente de vérification': 
            row?.isArchived ? 'Archivé':
            <>
    ...
    <Fragment>
      ...
      <Card>
        <CardBody>
          ...
          <h1 style={{fontWeight: 'bold', marginLeft: '1rem'}}>LISTE DES ARTICLES ARCHIVES({(store.total ?? 0) }) </h1>
        </CardBody>
      </Card>
      <Card>
