...
function FullProductList() {
  ...
  return <div className="product-full-list">
    <div className="w-max-width w-full">
      <PageBanner label={q? `Recherche pour: "${q}"`: ``} loading={loading} />
        <CardCarousel show={5} >
            {categorie?.subCategories?.map((cat: any, key: number) => {
                return <div className=" relative w-full h-40 bg-contain bg-center bg-no-repeat"
                ...
                style={{background: 'rgb(0, 0, 0, .5)', fontSize: 16, color: '#fff'}}
                >
                    {cat?.name}
                </div>
                </div>
      ...
      <div className="flex justify-between mt-10">
        <div className="w-1/4 filters">
          ...
          loading={loading}/>
          {/* <CategoryFilter loading={loading} categorie={categorie}/> */}
        ...
        <div className="w-3/4 pl-30 right-side">
          ...
          {loading ? <OnlineLoader width="400px" height="12px" />: <p>{_products.length} produits sur {itemCount} au total  pour ‘{`${q ?? categorie?.name}`}‘</p>}
            <div className="sort">
